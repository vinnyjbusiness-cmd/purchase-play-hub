...
import { Mail, Plus, Trash2, Edit, Send, Zap, Clock } from "lucide-react";
...
export default function Communications() {
  ...
  const handleSendEmail = async () => {
    if (!subject || !body || selectedRecipients.size === 0) {
      toast.error("Fill in subject, body, and select recipients");
      return;
    }
    setSending(true);
    // For now, we store as a toast notification since actual email requires SMTP configuration
    const recipientNames = [...selectedRecipients].map(uid => members.find(m => m.user_id === uid)?.display_name || uid).join(", ");
    toast.success(`Email prepared for: ${recipientNames}`, {
      description: "To send actual emails, configure an email provider (e.g. Resend, SendGrid) in settings.",
    });
    setSubject("");
    setBody("");
  ...
  const openAddRule = () => {
    setEditingRule(null);
    setRuleName("");
    setRuleTriggerType("unactioned_order");
    setRuleDays(3);
    setRuleDayOfWeek("monday");
    setRuleRecipients(new Set());
    setRuleSubject("");
    setRuleBody("");
    setRuleOpen(true);
  };
  ...
  const openEditRule = (rule: EmailRule) => {
    ...
    setRuleDays(rule.trigger_config?.days || 3);
    setRuleDayOfWeek(rule.trigger_config?.day_of_week || "monday");
    setRuleRecipients(new Set(rule.recipient_user_ids));
    setRuleSubject(rule.subject_template);
    setRuleBody(rule.body_template);
    setRuleOpen(true);
  };
  ...
  const handleSaveRule = async () => {
    ...
    const payload = {
      ...
      recipient_user_ids: [...ruleRecipients],
      subject_template: ruleSubject,
      body_template: ruleBody,
    };
              ...
              <div className="flex flex-wrap gap-2">
                {members.map(m => (
                  <label
                    key={m.user_id}
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-sm cursor-pointer transition-colors ${
                      ruleRecipients.has(m.user_id) ? "border-primary bg-primary/5" : "hover:bg-muted/50"
                    }`}
                  >
                    <Checkbox
                      checked={ruleRecipients.has(m.user_id)}
                      onCheckedChange={() => toggleRecipient(m.user_id, ruleRecipients, setRuleRecipients)}
                    />
                    {m.display_name}
                  </label>
                ))}
