...
export default function AssignPurchaseDialog({ orderId, eventId, orderCategory, orderQuantity, onClose, onAssigned }: Props) {
  ...
  useEffect(() => {
    async function load() {
      // Get existing order_lines count
      ...
      // Get inventory status for each purchase
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {p.category}{p.section ? ` · ${p.section}` : ""} · {sym(p.currency)}{p.unit_cost.toFixed(2)}/ea
                    </p>
                    <p className="text-xs text-muted-foreground">
